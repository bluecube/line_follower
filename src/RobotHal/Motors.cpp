...
void Motors::Motor::setup_encoder(IdfUtil::PinT pinA, IdfUtil::PinT pinB) {
    ...
    HAL_CHECK(pcnt_new_unit(&pcntUnitConfig, &encoder));

    // Watch points for counter accumulation
    HAL_CHECK(pcnt_unit_add_watch_point(encoder, -pcntLimit));
    HAL_CHECK(pcnt_unit_add_watch_point(encoder, pcntLimit));

    pcnt_glitch_filter_config_t glitchFilterConfig = {
        .max_glitch_ns = 10000, // About 20x shorter than the count signal at max RPM
    };
    HAL_CHECK(pcnt_unit_set_glitch_filter(encoder, &glitchFilterConfig));
    ...
    HAL_CHECK(pcnt_new_channel(encoder, &channelConfig, &channelA));
    HAL_CHECK(pcnt_channel_set_edge_action(channelA, PCNT_CHANNEL_EDGE_ACTION_INCREASE, PCNT_CHANNEL_EDGE_ACTION_DECREASE));
    HAL_CHECK(pcnt_channel_set_level_action(channelA, PCNT_CHANNEL_LEVEL_ACTION_INVERSE, PCNT_CHANNEL_LEVEL_ACTION_KEEP));
    ...
    HAL_CHECK(pcnt_new_channel(encoder, &channelConfig, &channelB));
    HAL_CHECK(pcnt_channel_set_edge_action(channelB, PCNT_CHANNEL_EDGE_ACTION_INCREASE, PCNT_CHANNEL_EDGE_ACTION_DECREASE));
    HAL_CHECK(pcnt_channel_set_level_action(channelB, PCNT_CHANNEL_LEVEL_ACTION_KEEP, PCNT_CHANNEL_LEVEL_ACTION_INVERSE));

    HAL_CHECK(pcnt_unit_enable(encoder));
    HAL_CHECK(pcnt_unit_clear_count(encoder));
    HAL_CHECK(pcnt_unit_start(encoder));
}
...
int32_t Motors::Motor::readEncoder() const {
    int count;
    HAL_CHECK(pcnt_unit_get_count(encoder, &count));
    return count;
}
